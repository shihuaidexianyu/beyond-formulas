...
这一点值得单独钉住，因为混淆的代价很实在。矩阵 \(\begin{bmatrix}1&-1\\-1&1\end{bmatrix}\) 带着负元素，特征值却是 \(0\) 和 \(2\)，半正定；矩阵 \(\begin{bmatrix}1&2\\2&1\end{bmatrix}\) 元素全为正，特征值是 \(3\) 和 \(-1\)，不半正定。按上面的 \(ac\ge b^2\) 复核也一样：前者 \(1\cdot1=1\ge(-1)^2\)，后者 \(1\cdot1=1<2^2\)。半正定描述的是二次型与谱，跟元素的符号没有对应关系。
