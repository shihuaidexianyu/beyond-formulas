...
\subsection{CCA 不分解一个视图，而是对齐两个视图}
...
CCA 找到的是两个表示间最强线性共同变化，不是两边共享的因果因素。若日期、季节或采样设备同时影响两个视图，canonical correlation 会忠实捕捉这种混杂。ICA 依赖独立与非 Gaussian 性来选择单个视图的方向，CCA 依赖跨视图相关来选择成对方向；它们并不是“比 PCA 更高级”的统一替代品，而是在回答不同可辨认问题。
