...
\begin{quote}
\textbf{先抓住这一点：}概率 PCA 与因子分析把低维潜变量经过线性映射再加噪声，变成可估计的生成模型。二者主要差在噪声协方差结构，因此对哪些变异算信号有不同判断。
\end{quote}
...
缩放和符号也要固定约定。若允许一般 latent covariance，\(\Lambda\) 与 latent 尺度还能相互补偿；设 \(\Cov(z)=I\) 正是在移除这部分冗余。可辨认性约束决定哪些参数陈述真正有意义，因而具有实质统计作用，不是排版选择。
