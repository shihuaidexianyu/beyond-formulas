...
这是本单元的全部内容：用条件独立把大分布压成小因子。压缩带来的好处是参数量与计算量一起下降，麻烦是那些声明必须为真——图上缺的每一条边都是一个断言，断言错了程序不会报错，只会让推断结果悄悄偏掉。
...
第一篇建立共同语言：有向图的分解与 d-分离，无向图的势函数与配分函数，两族图各自表达不了的独立性模式，以及精确推断有多贵怎样由树宽而非节点数决定。第二篇把这套语言放进联合 Gauss 的情形，那里协方差的零与精度矩阵的零分别对应边缘独立与条件独立，稀疏结构直接写在矩阵元素上。
