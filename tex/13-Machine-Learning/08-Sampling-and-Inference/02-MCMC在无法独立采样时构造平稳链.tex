...
\begin{quote}
\textbf{先抓住这一点：}MCMC 构造以目标分布为平稳分布的相关样本链。平稳性不保证从任意初值收敛，有限运行的诊断也只能发现失败，不能证明已经充分混合。
\end{quote}
...
却可能不可得，也难以直接从 posterior 独立采样。MCMC 通过构造一条长期访问频率为 \(\pi\) 的 Markov 链，绕开对归一化常数的直接计算。
