...
却可能不可得，也难以直接从 posterior 独立采样。MCMC 通过构造一条长期访问频率为 \(\pi\) 的 Markov 链，绕开对归一化常数的直接计算。它构造以目标分布为平稳分布的相关样本链。平稳性不等于从任意初值收敛，有限运行的诊断也只能发现失败，不能证明已经充分混合。
...
对 \(x\) 求和或积分便得到 \(\pi K=\pi\)，所以 \(\pi\) 是平稳分布。detailed balance 是平稳性的充分条件，不是必要条件；更重要的是，''平稳''只表示若当前已经服从 \(\pi\)，一步后仍服从 \(\pi\)，并不自动意味着从任意初值都能到达它。

要从初值收敛并让时间平均有效，还需要不可约、非周期、recurrence 等适当遍历条件。满足相应 Markov chain ergodic theorem 时，

\[
\frac1n\sum_{t=1}^nh(X_t)
\longrightarrow\E_\pi[h(X)]
\]
...
相邻链状态相关。记 \(\sigma_h^2=\Var_\pi(h(X))\)，\(\rho_k=\Corr(h(X_t),h(X_{t+k}))\) 为滞后 \(k\) 的自相关，则平稳链均值 \(\bar h_n=\frac1n\sum_th(X_t)\) 的方差近似为
...
对一个相关二维 Gaussian target 运行不同尺度的随机游走 Metropolis，比较接受率、每坐标 ESS 和均值的 Monte Carlo 标准误，可以看到：提议尺度取得极小时接受率接近一，链却几乎原地不动，ESS 很低、标准误很大。接受率最高的设置恰是效率最低的设置之一。

随后换成双峰 target，从两个峰分别启动链。两条链各自收敛于一个峰，\(\widehat R\) 明显大于一并报警；而任何一条单链的 trace 都平稳得毫无破绽。``删掉前一半样本''不能解决这种跨峰失败：burn-in 只删除仍困在同一峰内的样本，从不创造跨峰移动。
