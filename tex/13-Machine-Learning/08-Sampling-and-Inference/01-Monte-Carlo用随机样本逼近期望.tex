...
若选择适当 \(c\)，\(g\) 与 \(h\) 的共同波动被抵消。修正项期望为零，估计保持无偏；把方差写成 \(c\) 的函数，即 \(n^{-1}\bigl(\sigma_h^2-2c\operatorname{Cov}(h,g)+c^2\sigma_g^2\bigr)\)，这是关于 \(c\) 的二次函数，在 \(c^*=\operatorname{Cov}(h,g)/\sigma_g^2\) 处取最小，\(g\) 与 \(h\) 相关越强，方差下降越多。分层抽样保证不同区域都被覆盖，antithetic sampling 则利用负相关样本抵消波动。这些方法保持目标期望不变，同时在相同计算预算下减少方差。
