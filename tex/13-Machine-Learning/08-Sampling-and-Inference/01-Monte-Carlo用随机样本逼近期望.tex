...
也可能因维度高和 \(h\) 非线性而无法解析求出。规则网格在每个维度取 \(m\) 个点需要 \(m^d\) 次计算；Monte Carlo 改用随机样本——计算误差通常按 \(1/\sqrt n\) 缩小，与维数无关。
...
它把积分问题转成了平均问题。只要 \(\E|h(X)|<\infty\)，大数定律给出一致性；记 \(\sigma_h^2=\Var(h(X))\)，若 \(\sigma_h^2<\infty\)，则
...
若事件非常罕见，普通模拟可能几乎看不到失败，估计值甚至为零。此时风险依然存在，只是采样分布把计算资源浪费在大量普通情形上。
...
若选择适当 \(c\)，\(g\) 与 \(h\) 的共同波动被抵消。修正项期望为零，估计保持无偏；把方差写成 \(c\) 的函数，即 \(n^{-1}\bigl(\sigma_h^2-2c\Cov(h,g)+c^2\sigma_g^2\bigr)\)，这是关于 \(c\) 的二次函数，在 \(c^*=\Cov(h,g)/\sigma_g^2\) 处取最小，\(g\) 与 \(h\) 相关越强，方差下降越多。分层抽样保证不同区域都被覆盖，antithetic sampling 则利用负相关样本抵消波动。这些方法保持目标期望不变，同时在相同计算预算下减少方差。
