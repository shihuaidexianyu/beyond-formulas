...
空间污染浓度插值把同一选择推向更高风险。RBF 的长度尺度较大（即 \(\gamma\) 较小）时，公路旁很窄的污染峰具有很大的 RKHS 代价，容易被平滑掉；长度尺度很小时，模型又可能把传感器噪声解释成真实起伏。\(\lambda\) 只是在\emph{已经选定的核几何内}限制范数，替代不了尺度选择。若决策关心是否越过局部安全阈值，仅用全区域平均预测误差来选核与 \(\lambda\)，可能恰好掩盖最需要保留的那个窄峰。
...
\node[black!55,font=\footnotesize,anchor=east] at (-1.6,-1.15)
...
代价则第一次浮出水面：解的规模跟着样本数走，而不是跟着特征维数走。\hyperref[sec:13-Machine-Learning/09-Kernel-Methods-and-Gaussian-Processes/04]{``核岭回归与核方法的代价''}把定理用于平方损失推出闭式解，并把这笔账算清楚。
