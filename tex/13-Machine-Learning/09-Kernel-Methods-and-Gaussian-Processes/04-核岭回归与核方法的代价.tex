...
上一节的表示定理许诺：``经验损失加范数惩罚''的无穷维问题必然塌缩成 \(n\) 维问题。本节先兑现这个许诺——用于平方损失，推出核岭回归的闭式解；再把它与\hyperref[sec:13-Machine-Learning/02-Linear-Models/02]{``正则化从 Ridge 到 Lasso''}中的原始形式对照，看清同一件事的两种视角；最后清点账单：Gram 矩阵的存储与求逆代价划定了朴素核方法的规模边界。核方法消除了对特征维数的显式依赖，却把计算负担转移到了样本数：训练要处理 \(n\times n\) 的 Gram 矩阵，预测还要重新访问许多训练点。核化不是免费升维，而是用样本空间中的二次存储与三次求解交换特征空间的表达力。
...
\(\lambda\) 控制范数预算。正则项 \(\lambda n\,\alpha^{\trans}K\alpha\) 直接压在 \(\norm{f}_{\mathcal{H}_k}\) 上：\(\lambda\to0\) 时模型几乎只追求拟合，若 \(K\) 正定则退化为插值，训练误差为零；\(\lambda\to\infty\) 时 \(\widehat\alpha\to0\)，模型塌成常数。中间地带是偏差与方差的权衡，与\hyperref[sec:13-Machine-Learning/02-Linear-Models/02]{``正则化从 Ridge 到 Lasso''}中 \(\lambda\) 的角色一致，只是这里收缩的是 RKHS 几何下的``不光滑方向''——对应 \(K\) 的小特征值方向——而非坐标方向的系数。
...
选择它们的纪律与\hyperref[sec:13-Machine-Learning/02-Linear-Models/02]{``正则化从 Ridge 到 Lasso''}强调的完全相同：预处理、核的选择与超参数的调节都必须包在每个训练折内部，用嵌套验证或从未参与选择的测试集评估整个流程。核化没有改变评估纪律，只改变了被评估的对象。
...
\tightlist
\item
  Nyström 近似：抽样 \(m\ll n\) 个锚点，只用 \(K\) 的对应列做低秩近似 \(K\approx CW^{-1}C^{\trans}\)，把存储与求解降到 \(O(nm)\) 与 \(O(m^{3})\) 量级；
\item
...
如果只是为了拟合训练数据，核方法并非必需；真正为它定价的是泛化。\hyperref[ch:118]{``统计学习理论：训练误差对真风险有多乐观''}一章的核心问题是训练误差比真风险乐观多少，答案取决于所用函数类的有效容量。对核方法，容量恰好由范数球
