...
上一节的表示定理许诺：``经验损失加范数惩罚''的无穷维问题必然塌缩成 \(n\) 维问题。本节先兑现这个许诺——用于平方损失，推出核岭回归的闭式解；再把它与\hyperref[sec:13-Machine-Learning/02-Linear-Models/02]{``正则化从 Ridge 到 Lasso''}中的原始形式对照，看清同一件事的两种视角；最后清点账单：Gram 矩阵的存储与求逆代价划定了朴素核方法的规模边界，而 \(\norm{f}_{\mathcal{H}_k}\) 与泛化理论的联系则解释了为什么这笔账值得付。

\begin{quote}
\textbf{先抓住这一点：}核方法消除了对特征维数的显式依赖，却把计算负担转移到了样本数：训练要处理 \(n\times n\) 的 Gram 矩阵，预测还要重新访问许多训练点。核化不是免费升维，而是用样本空间中的二次存储与三次求解交换特征空间的表达力。
\end{quote}
