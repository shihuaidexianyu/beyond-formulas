...
\(\lambda\) 控制范数预算。正则项 \(\lambda n\,\alpha^{\trans}K\alpha\) 直接压在 \(\norm{f}_{\mathcal{H}_k}\) 上：\(\lambda\to0\) 时模型几乎只追求拟合，若 \(K\) 正定则退化为插值，训练误差为零；\(\lambda\to\infty\) 时 \(\widehat\alpha\to0\)，模型塌成常数。中间地带是偏差与方差的权衡，与``正则化从 Ridge 到 Lasso''中 \(\lambda\) 的角色一致，只是这里收缩的是 RKHS 几何下的``不光滑方向''——对应 \(K\) 的小特征值方向——而非坐标方向的系数。
