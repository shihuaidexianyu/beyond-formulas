...
Logistic 回归里完全可分的数据是同一件事的高维版本：存在一个方向，沿它放大系数能让似然持续增加，优化器于是一路把范数推大。日志上看到系数发散、迭代不收敛，第一反应不该是换优化器或调步长，而是检查经验矩是不是贴上了边界。加先验、加 \(\ell_2\) 正则、或者换一个不那么饱和的模型都能把解拉回有限区域，只是估计的对象已经不再是原来那个。
