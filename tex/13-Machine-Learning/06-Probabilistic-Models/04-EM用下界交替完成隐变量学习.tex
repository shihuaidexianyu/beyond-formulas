...
因为两个 \(\log q(z)\) 项抵消，联合密度与条件密度之比恰好留下 \(p(x\mid\theta)\)，而它对 \(z\) 是常数，期望不起作用。
\end{proposition}

KL 散度非负，所以 \(\mathcal L\) 是 log-likelihood 的下界，也称证据下界（evidence lower bound, ELBO）。这个恒等式给 EM 两步以明确含义：一边让下界贴近真实目标，一边抬高这个下界。
...
\sum_i\gamma_{ik}
(x_i-\mu_k^{\text{new}})
(x_i-\mu_k^{\text{new}})^{\trans}.
\]

若 \(\gamma_{ik}\) 只有 0 和 1，这些式子就是按已知类别分组的普通均值与 covariance。EM 用 posterior 权重保留归属不确定性，硬猜标签再重训会丢掉这部分信息。

例如用振动与温度数据区分机台运行工况时，\(\gamma_{ik}=0.8\) 可以读作：在当前混合模型与参数下，这条记录对第 \(k\) 个成分有较高 posterior responsibility。它不能直接改写成``机台有八成概率处于某个真实故障状态''。真实工况可能连续漂移，一个偏斜工况也可能被几个 Gaussian 成分共同近似；反过来，两个物理状态若在现有传感器上重叠，也可能被合成一个成分。
