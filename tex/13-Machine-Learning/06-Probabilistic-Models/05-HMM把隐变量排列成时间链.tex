...
\begin{quote}
\textbf{先抓住这一点：}HMM 用一阶 Markov 隐状态和条件独立观测把指数多条路径压成动态规划。Viterbi 最优路径与逐时刻最大后验状态通常不是同一个问题。
\end{quote}
...
把转移矩阵调得很``粘''——留在原状态的概率接近一——可以找到这样的参数设置：某时刻 Viterbi 路径经过的状态并不是该时刻 marginal posterior 最大的状态，逐点最优拼出的并非整体最优路径。把序列复制到很长，普通概率域的连乘会下溢为零；缩放版与 log-domain 版则给出一致且有限的 log-likelihood。
