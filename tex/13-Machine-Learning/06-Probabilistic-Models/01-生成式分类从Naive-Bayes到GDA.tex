...
写下联合分布之后，采样、补缺失、给不确定性都变成同一个模型上的不同查询。代价也是明确的：你必须为每一个特征维度作出分布假设，而假设越多，出错的地方越多。
...
\node[font=\scriptsize,anchor=west] at (0.45,0.62) {\(p(x\given Y=k)\)};
...
这就是上图左侧那两团等高线。covariance 决定椭球的朝向和各方向的尺度，于是``离哪个类更近''不再由欧氏距离决定，而由 Mahalanobis 距离决定：沿方差大的方向走同样远，付出的代价更小。
