...
这个模板就是生成式建模。判别式模型学的是从输入到输出的那条映射，它只回答被问到的那个问题，省力而稳健。生成式模型写下的是完整的联合分布：类别怎样出现、隐状态怎样转移、观测怎样从状态里产生。写下联合分布之后，采样、补缺失、给不确定性都变成同一个模型上的不同查询，标签缺失时也还有话可说。换来这些的前提同样明确——你要为所有变量作出分布假设，假设越多，能出错的地方越多。
