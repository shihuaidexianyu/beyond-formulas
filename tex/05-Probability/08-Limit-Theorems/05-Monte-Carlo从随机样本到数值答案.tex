...
\begin{quote}
\textbf{先抓住这一点：}Monte Carlo 用抽样把积分改写成平均，误差通常按 \(1/\sqrt n\) 缩小。可信结果还必须确认抽样目标正确、方差可控并处理样本相关性。
\end{quote}
...
\qquad p=\frac\pi4.
\]
这个例子直观，却不是计算 \(\pi\) 的高效方法。它的教学价值在于展示怎样把几何面积变成事件概率；它也反向使用了\hyperref[sec:05-Probability/08-Limit-Theorems/02]{``大数定律与长期平均''}一节建立的同一联系：那里由模型概率 \(p\) 预言长期频率，这里由长期频率反推未知的 \(p\)。

\subsection{方差缩减比盲目加样本更聪明}

Monte Carlo 误差常数由 \(\Var(g(X))\) 决定。除了增大 \(n\)，还可以改变估计器而保持目标期望：
...
伪随机数生成器要有足够质量；马尔可夫链 Monte Carlo（MCMC）样本通常相关，不能把样本数 \(n\) 当作 \(n\) 个独立观察。对平稳链和某个具体估计函数，常用近似为
...
\]
其中 \(\rho_k\) 是该函数值序列的滞后 \(k\) 自相关。MCMC 中常见的正自相关通常使 \(n_{\mathrm{eff}}<n\)，但这不是逻辑必然：负自相关可能使某些估计量的有效样本量超过 \(n\)。实际计算还要稳定估计或截断这串自相关和，不能把单个通用 ESS 当作整条链的固有属性。
此外还要区分：
...
样本平均非常稳定地收敛到错误目标，仍然是错误答案——这与\hyperref[sec:05-Probability/08-Limit-Theorems/02]{``大数定律与长期平均''}一节列出的选择偏差、共同系统误差同源：重复只能缩小随机误差，不能修正抽样机制本身。
