...
那句话是：\(q\) 能写成若干个线性表达式的平方和，而且这组表达式不留零方向。特征值版本用正交基做这件事，\(q=\sum\lambda_iy_i^2\) 里的 \(y=Q^{\trans}x\)；\(A=B^{\trans}B\) 版本干脆把平方和写成 \(q=\norm{Bx}^2\)，列满秩正是``\(Bx=0\) 只有零解''，也就是不留零方向；\(A=LL^{\trans}\) 版本把这组线性表达式取成三角的，是下一节的主题；顺序主子式与主元版本则是消元过程的记账——消元实际上在做逐步配方，第 \(k\) 个主元就是第 \(k\) 个平方项的系数，而顺序主子式之比恰好等于主元。换基的方式不同，能不能写成没有零方向的平方和这件事不变。
