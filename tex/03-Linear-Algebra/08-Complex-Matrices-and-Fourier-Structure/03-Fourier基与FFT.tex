...
\begin{tikzpicture}[x=1cm,y=1cm,font=\footnotesize,>=stealth]
  ...
  \begin{scope}[shift={(6.4,0)}]
    \draw[black!40] (-0.3,0) -- (5.0,0);
    \draw[blue!55!black,thin] plot[domain=0:4.55,samples=80] (\x,{1.3*cos(deg(2*pi*\x/5.2))});
    \foreach \j in {0,...,7} {
      \fill[blue!65!black] ({0.65*\j},{1.3*cos(45*\j)}) circle (2pt);
      \draw[blue!65!black] ({0.65*\j},0) -- ({0.65*\j},{1.3*cos(45*\j)});
    }
    \draw[red!55!black,thin] plot[domain=0:4.55,samples=140] (\x,{-1.3*cos(deg(6*pi*\x/5.2))});
    \foreach \j in {0,...,7} {
      \fill[red!60!black] ({0.65*\j},{-1.3*cos(135*\j)}) circle (2pt);
      \draw[red!60!black] ({0.65*\j},0) -- ({0.65*\j},{-1.3*cos(135*\j)});
    }
...
这三节做的其实是同一件事：先确认标量域必须是 \(\C\)，再确认内积必须带共轭，然后让谱定理在这套语言里重新落地，最后发现平移结构会把特征基唯一确定下来。离散情形到此为止。把序列换成函数、把循环移位换成平移算子之后，同样的推理给出连续 Fourier 变换与谱方法，那是\hyperref[ch:068]{``Fourier 与谱方法''}的内容；而复特征值的模长与幅角怎样支配一个系统的振荡与衰减，会在\hyperref[ch:132]{``稳定性与动力系统''}里被反复调用。
