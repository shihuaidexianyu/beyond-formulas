...
对所有 \(H\) 成立，所以这个目标是凸的。全程没有出现四阶数组，也没有出现一个下标。注意 \(\mathcal H_X[H]=A^{\trans}AH\) 这个式子本身就是可以直接编码的：给一个 \(H\)，做两次矩阵乘法即得，代价与一次前向相当。
...
这个例子给出的判断值得单独记下：二阶近似并非越完整越好。一个结构化的近似可以换来正定性、更低的成本和更稳的方向，代价是它只在某个区域内可信，而那个区域必须说清楚。深度学习里流行的 K-FAC、各种对角或 Kronecker 近似，走的都是同一条路子。
