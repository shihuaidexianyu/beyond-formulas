...
这个界值得看一眼：右端没有 \(R\)。若不带 \(e^{iaz}\)，只能用长度乘上界的粗估 \(\pi R\,M_R\)，要它趋零就得 \(M_R=o(1/R)\)——\(1/(1+z^2)\) 满足，但 \(1/(1+z)\) 就不满足了。指数因子把长度因子 \(\pi R\) 整个吃掉，代价是要求 \(a>0\) 且必须封在上半平面。这就是 Jordan 引理的全部内容：它不是一个技术性补丁，它是``为什么可以只数几个留数''的理由。
