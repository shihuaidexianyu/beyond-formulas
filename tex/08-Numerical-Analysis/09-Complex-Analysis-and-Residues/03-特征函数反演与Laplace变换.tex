...
看到 Fourier 积分就想封围道，这里会当场翻车：\(e^{-\abs t}\) 在 \(t=0\) 有折角，没有跨越原点的解析延拓，任何绕过原点的围道都无从谈起。正确做法是在 \(t=0\) 处把正负半轴分开，各自是一个能收敛的初等积分：
...
\begin{tikzpicture}[scale=0.9,font=\small,>=stealth]
  ...
  \begin{scope}[shift={(6.9,1.75)}]
    ...
    \node[right,font=\footnotesize,blue!55!black] at (3.45,0.1) {\(e^{-2.2x}\)：迅速消失};
  \end{scope}
  \begin{scope}[shift={(6.9,0)}]
    \draw[black!35] (0,0) -- (3.3,0);
    \draw[orange!80!black,thick] plot[domain=0:3.3,samples=120] (\x,{0.6*exp(-0.7*\x)*cos(4.2*\x r)});
    \node[right,font=\footnotesize,orange!70!black] at (3.45,0.1) {\(e^{-0.7x}\cos(4.2x)\)：慢衰减的振荡};
  \end{scope}
  \begin{scope}[shift={(6.9,-1.75)}]
    \draw[black!35] (0,0) -- (3.3,0);
...
沿这条线还能读出刚性的来源。刚性方程的特征值实部相差几个数量级，翻译成极点，就是有的极点远在左边（对应飞快衰掉的分量），有的贴着虚轴（对应慢慢演化的分量）。显式格式的步长被最左边那个极点卡住，而人们真正关心的是最右边那个。刻度差得越远，格式越难做。
