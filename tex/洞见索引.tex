...
这一章是查阅用的：一句洞见加几个入口，供你在需要时快速定位。想把其中几个动作真正读透——它们在哪些地方换了名字出现、每次成立各自靠什么条件、条件塌掉时会以什么形态失败——书末的\hyperref[ch:145]{``反复出现的想法：同一个动作在十六个学科里换了名字''}把线性化、换基、迭代、不变量、换序这五条展开成了完整的正文。两处不必都读，但读完那一章之后再回来看这份索引，它会从一张清单变成一张地图。
