...
\begin{quote}
\textbf{先抓住这一点：}反向传播不构造完整 Jacobian。它从标量损失出发，沿计算图反向传播当前目标真正需要的向量--Jacobian 乘积。
\end{quote}
...
逐样本看，\(Z_2\) 的第 \(i\) 行只参与第 \(i\) 个样本的损失，该样本对 \(\nabla_{W_2}L\) 的贡献是外积 \(h_ig_{2i}^{\trans}\)（\(h_i^{\trans}\)、\(g_{2i}^{\trans}\) 分别为 \(H\) 与 \(G_2\) 的第 \(i\) 行）。对 \(i\) 求和恰是矩阵乘积，于是第二层梯度为

\[
\nabla_{W_2}L=H^{\trans} G_2,
\qquad
...
把标量例子改为两个隐藏单元，让两个单元的输出都进入同一个最终节点，就得到一个多路径计算图。手算每条路径对 \(x\) 的贡献并相加，所得总导数与自动微分的结果一致——多路径贡献必须累加，这正是框架在计算图上做的事。

接着实现上面的 batch 网络，核对梯度不必逐元素检查所有参数：对一个随机方向做一次中心有限差分即可。directional check 之所以有效，是因为 transpose 与 reduction 类错误几乎总会破坏梯度与任意方向的内积关系；换一个随机方向再查一次，漏检的可能进一步降低。
