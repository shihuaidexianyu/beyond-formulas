...
新问题的解会明显偏向小模型。原因直接写在式子里：\(N\) 在两项里都出现，\(D\) 只在一项里出现，所以增大 \(N\) 的边际代价被 \(T\) 放大，而增大 \(D\) 的代价是一次性的。把一个较小的模型训得远超训练最优的 token 数，从训练账本上看是浪费，从总账上看可能便宜得多。这正是许多面向部署的模型选择``小模型、超长训练''的原因，它不是妥协，是换了目标函数之后的最优解。
