...
\emph{两幅图的起点和终点完全一样：信号系数从 \(1\) 降到 \(0\)，噪声系数从 \(0\) 升到 \(1\)。区别只在中间怎么走。线性调度的信号曲线一头栽下去，橙色区段（两条曲线量级相当、监督信号最有信息量的地方）只占链长的一小段；余弦调度把交叉点推到中央，同一个区段拉宽了一半以上。调度不改变终点，只改变训练样本落在哪里。}
\end{center}
