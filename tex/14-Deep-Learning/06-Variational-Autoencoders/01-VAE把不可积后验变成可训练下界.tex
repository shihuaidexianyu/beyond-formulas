...
\subsection{下界写成之后，还有一个退化解等在那里}

这个下界有一处值得先记下来的性质：它允许一个把潜变量彻底闲置的解。如果 decoder 本身足够强，它可以几乎不看 \(z\) 就把数据拟合得不错，此时让 \(q_\phi(z\given x)\) 一直贴着先验、KL 项归零，反而比在潜变量里编码信息更划算——目标函数并没有惩罚这个选择。这就是 posterior collapse，它不是需要抓的虫子，是我们写下的目标本身允许的最优解。

这件事的分量足够单独占一篇。\hyperref[sec:14-Deep-Learning/06-Variational-Autoencoders/03]{``后验坍缩与表示能力的失衡''}会把它讲完：为什么它是 ELBO 的结构性后果而不是训练技巧问题，怎样从逐维 KL 的分布上诊断出来，以及四类缓解手段各自动的是目标、路径、约束还是模型容量。
