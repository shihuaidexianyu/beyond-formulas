一块盘坏了。在存着几 PB 数据的集群里，这算不上事故，只是日常——磁盘、整机乃至整个机架都会以可预测的频率退出服务。早年的对策很直接：每份数据存三副本，坏掉一份就从另外两份拷回来，代价是存储成本乘以三。
...
若只想解决眼前的一个问题，也不必顺读。想弄清冗余为什么真能纠错，第二篇够了；想要一个确定的、可以手工验证的方案，读第一、三篇；关心长码在现实成本下怎样解码，直接进第四篇，它用到的 Tanner 图建立在\hyperref[ch:029]{``图论：局部邻接怎样形成全局网络''}的二分图语言上。
