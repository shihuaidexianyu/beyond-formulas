...
上一节用采样回报 \(G_t\) 当 \(Q^\pi\) 的无偏样本，代价已经看到了。换个做法：学一个 \(V_\phi\approx V^\pi\)，用它就地构造权重。减去状态基线得到优势 \(A^\pi(s,a)=Q^\pi(s,a)-V^\pi(s)\)，而一步时序差分
...
加权，多步展开则要乘上一串比率。上一节说过这串乘积会怎样集中，V-trace 之所以对两个阈值分开处理，正是因为其中一个决定不动点对应哪个策略，另一个控制迹沿轨迹传播的强度。在 Actor--Critic 里这笔账要付两次：Critic 的目标需要校正到 \(\pi_\theta\) 上，Actor 的梯度也需要，两处的截断偏差会叠加。
