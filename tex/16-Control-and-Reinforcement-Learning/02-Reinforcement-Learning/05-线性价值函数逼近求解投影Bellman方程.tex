...
确实是回归目标的随机梯度步，没有自举，代价是等待与方差。一步 TD 换成
...
再解 \(\widehat Aw=\widehat b\)。这就是 LSTD，它比逐步 TD 更充分地用掉每个样本，代价是维护一个 \(m\times m\) 矩阵，以及承担它的条件数。实现时应当解线性系统而不是显式求逆，理由与\hyperref[sec:08-Numerical-Analysis/07-Numerical-Methods-for-ML/03]{``需要的是解，而不是逆矩阵''}里说的完全一致。样本相关、覆盖不足或特征近共线都会让 \(\widehat A\) 病态，此时正则化不是可选项。
